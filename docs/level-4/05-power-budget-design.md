...
A real power budget has to account for each operating mode's actual current draw as a physical quantity, not a percentage: deep sleep draws single-digit to tens of microamps (RTC domain only, as covered earlier), active idle with Wi-Fi connected but not transmitting draws tens of milliamps (CPU plus radio in receive/listen mode for beacons), and active transmission draws hundreds of milliamps in short bursts — a battery-powered design's actual runtime is dominated almost entirely by how long the device spends in the *highest* current-draw state and how often, which is why waking briefly, transmitting, and returning to deep sleep as fast as possible (rather than staying "awake but idle" between transmissions) is the single highest-leverage firmware decision for battery life, since idle-but-connected current draw over minutes can dwarf the energy cost of the transmission burst itself.

Battery chemistry interacts with this at the hardware level too: a battery's own internal resistance limits how much instantaneous current it can source before its terminal voltage sags — this is exactly the same brownout mechanism from device design and diagnostics, but now the "weak power supply" is the battery's own aging or chemistry rather than a bad regulator, which is why coin-cell-powered designs (high internal resistance) generally cannot tolerate a bursty high-current radio transmission at all without a supercapacitor or bulk capacitor buffering the transient locally, while a lower-internal-resistance LiPo pack can source the same transient current directly from the cell.
